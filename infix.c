...
precedence(str[i])>precedence(s1.a[s1.tos])) //if str[i] is a operator
{
//stack is empty push
...
{
//if str[i] is operator having less or equal precedence than stack top operator,
...
{
post[j]=pop(&s1); //pop operatorsfrom stack having higher or equal precedence//make it a part ofoutput string
j++;
